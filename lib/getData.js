...
import { getPostsPerPage, urlize } from "./utilities";
...
function getAllAuthorIDs(dataType) {
  const sortedData = getSortedData(dataType);
...
function getAllPageNbrs(dataType, postsPerPage = getPostsPerPage()) {
...
function getTotalPages(dataType, postsPerPage = getPostsPerPage()) {
  const fileNames = getAllFilenames(dataType);
...
function getAuthorPostsFromAuthorID(authorID, dataType) {
...
function getSingleContentFromSlug(slug, dataType, format = "toml") {
  const fileNames = getAllFilenames(dataType);
