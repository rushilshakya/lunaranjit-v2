...
function getDataForStaticPropsForPage(
  ...
  const hasPinnedPost = pinnedPost ? true : false;
  const remainingPosts = hasPinnedPost
    ? allPosts.filter((x) => x.id !== pinnedPost.id)
    : allPosts;
  const startIdx =
    pageNbr == 1 ? 0 : postsPerPage * (pageNbr - 1) - hasPinnedPost;
  const endIdx = postsPerPage * pageNbr - hasPinnedPost;
  ...
  return {
    ...
    pinnedPost: pageNbr == 1 && hasPinnedPost ? pinnedPost : null,
  };
}
